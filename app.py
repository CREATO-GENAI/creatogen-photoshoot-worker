...
def handler(event):
    try:
        ...
        denoise  = max(0.10, min(0.80, denoise))  # clamp
        guidance = float(inp.get("guidance_scale", 5.5))
        scale_cn = float(inp.get("controlnet_scale", 0.8))  # new
        seed     = inp.get("seed")
        ...
        with torch.inference_mode(), torch.autocast(device_type="cuda", dtype=DTYPE if DEVICE=="cuda" else torch.float32):
            result = PIPE(
                prompt=prompt,
        ...
        return {
            "variants": [
                {"image_b64": b64_png, "format": "png", "seed": int(seed) if seed else 0}
            ],
            "meta": {
                "width": W, "height": H,
                "guidance_scale": guidance,
